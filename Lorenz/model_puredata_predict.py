...
"""
fully data-driven model built just for prediction; no training option

"""


class lstm_data_pred(lstm_data):
	...
	def compile(self):
		...
		y0 = self.inputs[-1]
		y0, dy0dt = self.time_integrate(y0, x0)

		self.model_s = Model(inputs=self.inputs, outputs=[dy0dt, y0] + xlstm_states)
		...
		self.model_p = Model(inputs=self.inputs_p, outputs=dydt_list+y_list+xlstm_states)
	...
	def predict(self, inputs, steps, loadWeights=None):
		...
		dysdt, ys1, xs = s_outputs[0], s_outputs[1], s_outputs[2:]		# B_s, y_{s+1}
		dydt_pred, y_pred = [dysdt.copy()], [ys1.copy()]
		...
		for step in range(nb_runs):
			...
			dydtl, yl = p_outputs[:self.l], p_outputs[self.l:2*self.l]
			xs = p_outputs[2*self.l:]
			ys1 = yl[-1]
...
def main():
	...
	loadFile = os.path.join(cwd, fname)
	npzfile = np.load(loadFile)
	...
	lstm_model = lstm_data_pred(trunc_dim, s, l, hid_units_x=nx, hid_units_B=nB, params=npzfile, savepath=sp)
	lstm_model.compile()

	steps = 4000
	test_inputs, traj_dict = lstm_data_pred.data_proc(trunc_dim, s, steps, loadFile, idx_start=9000)
	weights = './logs/lstm_data/test1/trained_weights.h5'
	y_pred, dydt_pred = lstm_model.predict(test_inputs, steps, loadWeights=weights)
	case_idx = 400
	...
	plt.savefig('./figures_performance/data_tc'+str(case_idx)+'_series.png', dpi=300)
