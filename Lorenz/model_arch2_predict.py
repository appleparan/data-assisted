...
for 2-d POD on Lorenz, training on architecture 1 alone proves to be sufficient
...
def main():
	...
	trunc_dim = 1
	s, l = 25, 100
	sp = os.path.join(cwd, 'logs\\lstm_arch2\\test1\\')
	...
	steps = 50
	total_sample = 4000
	test_inputs, traj_dict = lstm_hybrid_pred.data_proc(trunc_dim, s, steps, loadFile, idx_start=total_sample)
	
	weights = os.path.join(sp, 'trained_weights.h5')    ### enter path for weights obtained using model_arch1 ###
	y_pred, B_pred, dydt_pred = lstm_model.predict(test_inputs, steps, loadWeights=weights)
	case_idx = l - 1
	tt = np.linspace(.01, .01*steps, steps)

	## plot predictions
	f, axarr = plt.subplots(1, lstm_model.fdim - trunc_dim)
	for dim in range(lstm_model.fdim - trunc_dim):
	...
	plt.savefig(lstm_model.savepath+'figures_performance_tc'+str(case_idx)+'_series.png', dpi=300)
